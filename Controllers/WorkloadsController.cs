using kiseleva_nastia_42_21.Interfaces.WorkloadInterfaces;
...
namespace kiseleva_nastia_42_21.Controllers
{
    [ApiController]
    [Route("[controller]")]
    public class WorkloadsController: ControllerBase
    {
        ...
        private readonly KiselevaDbContext _dbContext;
        public WorkloadsController(KiselevaDbContext dbContext, ILogger<WorkloadsController> logger, IWorkloadService workloadService)
        {
        ...
        [HttpPost("SearchProfessorsByLastName")]
        public async Task<IActionResult> SearchProfessorsByLastName([FromBody] ProfessorSearchFilter filter)
        {
            if (string.IsNullOrWhiteSpace(filter.FirstName))
            {
                return BadRequest("Фамилия не может быть пустой.");
            }

            var professors = await _dbContext.Set<Professor>()
                .Where(p => p.FirstName.Contains(filter.FirstName))
                .ToListAsync();

            if (!professors.Any())
            {
                return NotFound("Профессора с такой фамилией не найдены.");
            }
        ...
        [HttpPost("GetWorkloadsByNumberOfHours")]
        public async Task<IActionResult> GetWorkloadsByNumberOfHours([FromBody] NumberOfHourseFilter filter)
        {
            ...
            var workloads = await _workloadService.GetWorkloadByEducationSubjectForNumOfHourse(filter);
            return Ok(workloads);
        }
